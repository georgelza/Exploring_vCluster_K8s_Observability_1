...
Alertmanager receives firing alerts from Prometheus, deduplicates and groups them, and routes them to notification channels. In this stack it is configured to send alerts to a Slack webhook (**not fully implimented**).
...
To enable Slack notifications, replace the webhook URL in `alertmanager.yaml` as defined in `6.alertmanager-configmap.yaml`.
